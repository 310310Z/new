...
### ログイン機能
- **ユーザー認証**: ユーザーはメールアドレスとパスワードを使用してログインします。Supabaseの認証サービスを利用して、安全なログインプロセスを提供します。
- **ログイン後の遷移**: 認証成功後、ユーザーはトップ画面にリダイレクトされ、日記作成や閲覧が可能になります。

### タイトルと本文の記述
- ユーザーは日記のタイトルと本文を自由に記述できます。タイトルは50文字以内、本文には文字制限がないため、詳細な記録が可能です。

### 画像の挿入
- 日記に画像を添付することで、文章だけでは伝わりにくい記憶や出来事を視覚的に表現できます。JPEG/PNG形式のみを許可します。
...
### バックエンド: Supabase（PostgreSQL（データの保存）、認証（ユーザ認証）、ストレージ（画像の保存））
